...
const getQueryParamsFromArgs = (
  args: FetchArgs | string,
): Record<string, string> => {
  if (typeof args === "string") return getQueryParams(args);
  return getQueryParams(args.url ?? "");
};
...
export const handleMockRequest = async (
  ...
  const queryParams = getQueryParamsFromArgs(args);

  // Helper function to paginate data
  const paginateData = <T>(data: T[], page: number, limit: number) => {
  ...
  if (route === "dashboard/plans" && method === "GET") {
    ...
    if (page && limit) {
      return { handled: true, data: paginateData(MOCK_PLANS, page, limit) };
    }
  ...
  if (route === "dashboard/clients" && method === "GET") {
    ...
    if (page && limit) {
      return {
        handled: true,
        data: paginateData(MOCK_CLIENT_PLANS, page, limit),
      };
    }
  ...
  if (route === "dashboard/agents" && method === "GET") {
    ...
    if (page && limit) {
      return { handled: true, data: paginateData(MOCK_AGENTS, page, limit) };
    }
  ...
  if (route === "dashboard/users" && method === "GET") {
    ...
    if (page && limit) {
      return { handled: true, data: paginateData(MOCK_USERS, page, limit) };
    }
  ...
  if (route === "dashboard/waiting-list" && method === "GET") {
    ...
    if (page && limit) {
      return {
        handled: true,
        data: paginateData(MOCK_WAITING_LIST, page, limit),
      };
    }
