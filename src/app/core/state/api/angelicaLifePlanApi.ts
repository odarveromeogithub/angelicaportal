...
interface AngelicaLifePlanResponse {
  success: boolean;
  message: string;
  data?: {
    planId: string;
...
export const angelicaLifePlanApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAngelicaLifePlans: builder.query<AngelicaLifePlanListItem[], { userId?: string } | undefined>({
      query: (params) => ({
    ...
    submitAngelicaLifePlan: builder.mutation<AngelicaLifePlanResponse, IAngelicaLifePlanFormData>({
      query: (data) => ({
    ...
    saveDraftAngelicaLifePlan: builder.mutation<AngelicaLifePlanResponse, Partial<IAngelicaLifePlanFormData>>({
      query: (data) => ({
    ...
    updateAngelicaLifePlan: builder.mutation<AngelicaLifePlanResponse, { id: string; data: Partial<IAngelicaLifePlanFormData> }>({
      query: ({ id, data }) => ({
        url: `angelica-life-plan/${id}`,
        method: 'PUT',
        body: data,
      }),
      invalidatesTags: (result, error, { id }) => [{ type: 'AngelicaLifePlan', id }, 'AngelicaLifePlan'],
    }),
    ...
    // Upload ID document
    uploadIdDocument: builder.mutation<{ success: boolean; url: string }, FormData>({
      query: (formData) => ({
    ...
    // Upload signature
    uploadSignature: builder.mutation<{ success: boolean; url: string }, { planId: string; signature: string }>({
      query: (data) => ({
